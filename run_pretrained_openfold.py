...
def longest_common_substring(strs):
    ...
    for i in range(length):
        for j in range(length - i, -1, -1):
            candidate = shortest_str[i : i + j]
            if all(candidate in other for other in strs):
                return candidate
    return ""


def find_unique_parts(strs):
    """Extract parts of each string not in the common substring and chain them with hyphen."""
    common = longest_common_substring(strs)
    unique_parts = []
    for s in strs:
        parts = s.split(common)
        unique_parts.append("-".join(filter(None, parts)))
    return "-".join(unique_parts)


def combine_strings(strs):
    """Combine the logic to find the LCS and chain the unique parts of the strings."""
    if not strs:
        return ""
    common = longest_common_substring(strs)
    unique_chain = find_unique_parts(strs)
    return f"{common}{'_' if common else ''}{unique_chain}"
...
def main(args):
    ...
    for model, output_directory in model_generator:
        ...
        for (tag, tags), seqs in sorted_targets:
            ...
            if (Path(output_directory) / f"{output_name}_unrelaxed.pdb").exists():
